...
OUTPUT_DIR = OUTPUT_ROOT / "stlouis" / "airspaces"
CSV_DIR = (
    REFERENCE_DATA_ROOT
    / "faa"
...
FULL_REGION = geo.CHART_LONLAT_BBOX or (
    base.SECTIONAL_EXTENTS["lon_left"],
    base.SECTIONAL_EXTENTS["lon_right"],
    base.SECTIONAL_EXTENTS["lat_bottom"],
    base.SECTIONAL_EXTENTS["lat_top"],
)
...
def fallback_airport_for_class_e_record(
    ...
    for airport in airports:
        ...
        if not point_in_parts(lon, lat, record["parts"]):
            continue
        ...
        airport_name = str(airport.get("ARPT_NAME") or "").upper()
        for token in airport_name.replace("/", " ").replace("-", " ").split():
            if len(token) >= 5 and token in ident_text:
                score += 2.0
                break

        distance = base.haversine_nm(centroid_lat, centroid_lon, lat, lon)
        candidates.append((score, -distance, airport))
...
def build_features() -> dict[str, object]:
    ...
    chart_transform = calibrate_full_chart_transform(image, FULL_REGION, airports)
...
def related_airport_for_airspace(
    ...
    for airport in airports:
        class_info = airport.get("class_info", {})
        if class_info.get(wanted_flag) != "Y":
            continue
